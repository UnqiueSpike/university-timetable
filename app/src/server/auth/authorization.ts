...
import { userRoles, studentClasses, classes, timetableEntries } from "../db/schema";

export type Permission = "timetable.read.own" | "timetable.read.course" | "staff.access";
export type Grant = { permission: Permission; scopeType: "self" | "course"; courseId: string | null };
export type Principal = { id: string; displayName: string; email: string; emailVerified: boolean; grants: Grant[] };
export async function getPrincipal(db: Database, current: { id: string; name: string; email: string; emailVerified: boolean }): Promise<Principal> {
