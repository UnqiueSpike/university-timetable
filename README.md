...
当前完成第一步：可运行的应用工程与新版静态登录页。**尚未实现登录、注册、密码重置、设备记忆、会话或权限验证。** 表单只演示输入、密码显隐和提示，不发送或保存凭据；请勿用真实密码演示。
...
`src/server/auth`、`api`、`services`、`db`、`integrations` 随后续认证、数据库和业务接入按需建立，本阶段不创建空的服务端实现。
...
打开 <http://localhost:3000>。当前首页就是登录页。环境变量不是启动条件；需要本地配置文件时可执行 `cp .env.example .env.local`。`.env.local` 已被忽略，不应提交真实密钥。
...
GitHub Actions 在 push 和 pull request 上执行 `npm ci` 和 `npm run check`。CI 配置已提供，远程运行结果以 GitHub 为准。
